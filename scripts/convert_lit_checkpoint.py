...
def copy_weights_llama(
    ...
    for name, param in lit_weights.items():
        ...
        if "_orig_mod." in name:
            raise Exception("jwk: Is this required? Not in upstream")
...
def convert_lit_checkpoint(
    checkpoint_path: Path, output_path: Path, config_path: Path, axonn_patch: bool = False
) -> None:
    ...
    if "falcon" in config.name:
        ...
        copy_fn = partial(copy_weights_llama, config, untie_weights=untie_weights, axonn_patch=axonn_patch)

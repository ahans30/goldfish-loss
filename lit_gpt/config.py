...
class Config:
    ...
    neptune_noise_alpha: Optional[float] = None  # neptune noise not added if None / False-y
    ...
    @property
    def mlp_class_axonn(self) -> Type:
        # `self._mlp_class` cannot be the type to keep the config json serializable
        return getattr(lit_gpt.model_axonn, self._mlp_class)
...
####################
# EleutherAI Pythia -- 32k vocab size
####################
pythia = [
    # https://huggingface.co/EleutherAI/pythia-14m/blob/main/config.json
    dict(
        name="pythia-32kvocab-14m",
        hf_config=dict(org="EleutherAI", name="pythia-14m"),
        block_size=512,
        n_layer=6,
        n_embd=128,
...
for c in pythia:
    # "pythia-14m" and "pythia-31m" don't have deduped version
    if c["name"] in ("pythia-32kvocab-14m", "pythia-32kvocab-31m"):
        continue
    copy = deepcopy(c)
    copy["name"] = f"{c['name']}-deduped"
    copy["hf_config"]["name"] = f"{c['hf_config']['name']}-deduped"
...
###############
cosmo = [
    # https://huggingface.co/HuggingFaceTB/cosmo-1b/blob/main/config.json
    dict(
        name="cosmo-1b",  # Based on LLaMA
        hf_config=dict(org="HuggingFaceTB", name="cosmo-1b"),
        vocab_size=32000,
        n_layer=24,
        n_embd=2048,
        n_head=16,
...
tiny_llama = [
    ...
    dict(
        name="tiny-llama-190m-chatml",
        hf_config=dict(org="tomg-group-umd", name="tiny-llama-190m-chatml"),
        block_size=2048,
        vocab_size=32002,
        padded_vocab_size=32002,
        padding_multiple=64,
        n_layer=12,
        n_head=32,
        n_embd=1024,
    ...
    dict(
        name="tiny-llama-1.1b-assistant",
        hf_config=dict(org="tomg-group-umd", name="TinyLlama-1.1B-assistant"),
        block_size=2048,
        vocab_size=32002,
        padded_vocab_size=32002,
        padding_multiple=64,
